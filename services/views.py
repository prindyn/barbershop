...
class ChooseServiceView(View):
    ...
    def get(cls, request):
        services = Service.objects.all()
        return render(request, 'services/choose_service.html', {
            'title': 'Service',
            'services': services
        })
...
class ChooseBarberView(View):
    ...
    def get(cls, request):
        if request.session.has_key('service_id'):
            ...
            return render(request, 'services/choose_barber.html', {
                'title': 'Barber',
                'barbers': barbers
            })
...
class ChooseDateView(View):
    ...
    def get(cls, request):
        if request.session.has_key('barber_id'):
            ...
            return render(request, 'services/choose_date.html', {
                'title': 'Date',
                'calendar': calendar,
...
class BookingFinishView(View):
    ...
    def get(cls, request):
        if request.session.has_key('time_id'):
            ...
            return render(request, 'services/booking_finish.html', {
                'title': 'Complete',
